...
int ArrayMatrix::getNumOfVectCols(){
    return this->rows;
}
...
void ArrayMatrix::printResultToFile(string filename) {
    ofstream ofs(filename.c_str());
    for (int i = 0; i < this->arrayList->getNumElements(); i++) {
        MatrixElement *current = this->arrayList->getMatrixElement(i);
        if(current->value != 0) {
            ofs << current->col << endl;
        }
    }
}

void ArrayMatrix::printResult(HashTable *label) {
    for (int i = 0; i < this->arrayList->getNumElements(); i++) {
        MatrixElement *current = this->arrayList->getMatrixElement(i);
        if(current->value != 0) {
            Label currentLabel = label->getLabel(current->col);
            if(currentLabel.id == current->col) {
                cout <<currentLabel.id <<" "<< currentLabel.label << endl;
            }
            else {
                cout<<current->col<<endl;
            }
...
void ArrayMatrix::printResultToFile(HashTable *label, string filename) {
    ofstream ofs(filename.c_str());
    for (int i = 0; i < this->arrayList->getNumElements(); i++) {
        MatrixElement *current = this->arrayList->getMatrixElement(i);
        if (current->value != 0) {
            Label currentLabel = label->getLabel(current->col);
            if (currentLabel.id == current->col) {
                ofs <<currentLabel.id <<" "<< currentLabel.label << endl;
            } else {
                ofs << current->col << endl;
            }
        }
...
//        matrixC->setValueAtCoordinate(this->rows, i+1, this->getValueAtCoordinate(this->rows, i+1));
//    }
...
ArrayMatrix *ArrayMatrix::vectorMatrixMultiplication(ArrayMatrix *matrix) {
    if (!this->multiplicationCompatible(matrix)) {
        //cout<<"not mult compatible"<<endl;
        return nullptr;
    }
    ...
    for (int i = 0; i < this->arrayList->getNumElements(); ++i) {
        MatrixElement *current1 = this->arrayList->getMatrixElement(i);
        for (int j = 0; j < matrix->arrayList->getNumElements(); ++j) {
            MatrixElement *current2 = matrix->arrayList->getMatrixElement(j);

            if(current1->col == current2->row){
                double sum = current1->value * current2->value;
                matrixC->setValueAtCoordinate(
                        current1->row,
                        current2->col,
                        sum+matrixC->getValueAtCoordinate(current1->row, current2->col));
            }
        }
    }
    if ((this->rows == matrix->cols && (matrixC->getValueAtCoordinate(matrixC->rows, matrixC->cols) == 0))) {
...
void ArrayMatrix::unionSets(ArrayMatrix *matrix){
    for (int i = 0; i < matrix->arrayList->getNumElements(); i++) {
        MatrixElement *current = matrix->arrayList->getMatrixElement(i);
        if(current->value != 0) {
            this->setValueAtCoordinate(current->row, current->col, current->value);
...
//ArrayMatrix *ArrayMatrix::getRowVector(int row){
//    ArrayMatrix *rowVector = new ArrayMatrix;
//    for(int i = 0; i < this->arrayList->getNumElements(); i++){
//        MatrixElement *current = this->arrayList->getMatrixElement(i);
//        if(current->row == row){
//            rowVector->setValueAtCoordinate(1, current->col, current->value);
//        }
//    }
//    return rowVector;
//}
...
int ArrayMatrix::getRows() const {
    return rows;
}

int ArrayMatrix::getCols() const {
    return cols;
}
